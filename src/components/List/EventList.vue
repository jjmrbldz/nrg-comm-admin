<template>
    <div class="formgrid grid">
        <div class="field col-2">
            ...
            <StatusSelect class="mt-2" v-model="params.filter_te_status" />
        </div>
        <div class="field col-2">
            <label>{{ $store.getters['languageStore/translate']('nameLang') }}</label>
            <InputText type="search" v-model="params.filter_te_title" class="text-base text-color  p-2 border-1 border-solid surface-border border-round appearance-none outline-none focus:border-primary w-full" @keyup.enter="getList()" />
        </div>
        <div class="col"></div>
    ...
    <DataTable :value="list" scrollable class="mt-4" stripedRows :loading="loading">
        ...
        <Column :header="this.$store.getters['languageStore/translate'](`actionLang`)" style="min-width: 100px">
            <template #body="{ data }">
                <UpdateItemButton :updateProps="{itemID: data.event_id, getAPI: 'getEvent', getParams: 'id', modalHeader: 'Add/Edit Event', icon: 'mdi mdi-gift', width: '60vw', modalType: 'AddEditEvent', callback: () => getList(), totalItems:totalitems}" />
...
export default {
    ...
    watch:{
        ...
        'params.filter_te_status'(data){
            this.getList()
        }   
    },
    methods:{
        newItem(id) {
            this.$dialog.open(this.$modalComponent.AddEditEvent, {
                props: {
                    header: this.$store.getters['languageStore/translate']('Add/Edit Event'),
                    style: {
                        width: '60vw'
                    },
                    modal: true
        ...
        async getList() {
            this.loading = true
            try {
                const res       = await api.getEventsList(this.params)
