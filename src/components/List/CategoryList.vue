<template>
    <div class="grid p-2">
        <div class="formgrid grid">
            <div class="field col-4">
                ...
                <StatusSelect v-model="params.c_status" />
            </div>
            <div class="field col-6">
                <label>{{ $store.getters['languageStore/translate']('nameLang') }}</label>
                <InputText type="search" v-model="params.c_title" class="text-base text-color  p-2 border-1 border-solid surface-border border-round appearance-none outline-none focus:border-primary w-full" @keyup.enter="getList()" />
            </div>
        </div>
        <div class="col"></div>
    ...
    <DataTable :value="list" scrollable class="mt-4" stripedRows :loading="loading">
        ...
        <Column :header="this.$store.getters['languageStore/translate'](`actionLang`)" style="min-width: 100px">
            <template #body="{ data }">
                <UpdateItemButton :updateProps="{itemID: data.id, getAPI: 'getCategory', getParams: 'id', modalHeader: 'Add/Edit Category', icon: 'mdi mdi mdi-playlist-edit', modalType: 'AddEditCategory', callback: () => getList(), totalItems: totalRecords}" />
            </template>
        </Column>
